...
namespace ABankAdmin.Controllers
{
    public class BlacklistNumberController : Controller
    {
        ...
        string controllerName = "BlacklistNumber";
        ...
        public ActionResult Create(IconicBlacklistVM reqBlacklistVm)
        {
            ...
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    int usertypecodelimit = Convert.ToInt32(reqBlacklistVm.USERTYPECODE);
                    dbModel.UserTypeID = (from s in db.UserTypeModels where s.USERTYPE == reqBlacklistVm.USERTYPE && s.DEL_FLAG == false && s.USERTYPE_CODE_LIMIT == usertypecodelimit select s.ID).SingleOrDefault();
        ...
        public ActionResult Edit(int? id)
        {
            ...
            editBlacklist.PrefixBooking = (from s in db.UserTypeModels where s.ID == info.UserTypeID && s.DEL_FLAG == false select s.USERTYPE_CODE_LIMIT).SingleOrDefault().ToString();
            editBlacklist.USERTYPE = (from s in db.UserTypeModels where s.ID == info.UserTypeID && s.DEL_FLAG == false select s.USERTYPE).SingleOrDefault();
        ...
        public ActionResult Edit(IconicBlacklistVM reqBlacklistVm)
        {
            ...
            {
                int usertypecodelimit = Convert.ToInt32(reqBlacklistVm.USERTYPECODE);
                int UserTypeID = (from s in db.UserTypeModels where s.USERTYPE == reqBlacklistVm.USERTYPE && s.DEL_FLAG == false && s.USERTYPE_CODE_LIMIT == usertypecodelimit select s.ID).SingleOrDefault();

                if (ModelState.IsValid)
                {
                    
                    #region duplicate Booking 
                    if (db.IconicBlacklistModels.Where(c => c.BlackListNumber.ToLower().Trim().Replace(" ", "") == reqBlacklistVm.BlacklistNo.ToLower().Trim().Replace(" ", "") && c.DEL_FLAG == false && c.ID != reqBlacklistVm.ID).Count() != 0)
                    {
                    ...
                    int usertypelimit = Convert.ToInt32(reqBlacklistVm.USERTYPECODE);
                    dbModel.UserTypeID = (from s in db.UserTypeModels where s.USERTYPE == reqBlacklistVm.USERTYPE && s.DEL_FLAG == false && s.USERTYPE_CODE_LIMIT == usertypelimit select s.ID).SingleOrDefault();
                    ...
                    ViewBag.EditBlacklist = "\"" + dbModel.BlackListNumber + "\"";
